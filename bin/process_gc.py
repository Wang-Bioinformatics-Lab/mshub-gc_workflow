...
def main():
    ...
    parser.add_argument('time_unit', help='time_unit')
    parser.add_argument('clustered_mgf', help='clustered_mgf')
    parser.add_argument('clusterinfo', help='clusterinfo')
    ...
    filename_df.to_csv("filesummary.tsv", sep="\t", index=False)
    ...
    hdf5_filename = os.path.join(folder_path, "data.h5")
    ...
    if args.time_unit == "MIN":
        cmds.append([python_runtime, args.import_script, "-f", file_type_of_import, args.spectrum_folder, hdf5_filename, "--timeunits", "'min'"])
    else:
        cmds.append([python_runtime, args.import_script, "-f", file_type_of_import, args.spectrum_folder, hdf5_filename, "--timeunits", "'sec'"])

    #intra align
    cmds.append([python_runtime, args.align_script, hdf5_filename, "--h5writepath", "'sp2D'"])

    #noisefilter
    cmds.append([python_runtime, args.noise_script, hdf5_filename, "--h5readpath", "'sp2D'", "--h5writepath", "'spproc2D'", "--window", "6", "--frame", "50"])

    #inter align
    cmds.append([python_runtime, args.interalign_script, hdf5_filename, "--h5readpath", "'spproc2D'", "--h5writepath", "'spal2D'"]) #According to ipython

    #peak detect
    cmds.append([python_runtime, args.peakdetect_script, hdf5_filename, "--h5readpath", "'spal2D'", "--individual", "no", "--frag_pattern", "deconvolution"])

    #output detection
    cmds.append([python_runtime, args.export_script, hdf5_filename, folder_path])
    ...
    #cmds.append([python_runtime, args.vistic_script, "--outputfile", os.path.join("..", args.tic_html), "--display", "no", hdf5_filename])
    cmds.append([python_runtime, args.report_script, "--output_prefix", "gnps-gc", hdf5_filename, "summary_temp"])
    cmds.append(["tar", "-cvf",  "summary.tar", os.path.join("summary_temp", "gnps-gc")])
    ...
    # This was here. If desired to cluster spectra add parameters CLUSTER_SPECTRA and RT_TOLERANCE to the function and the workflow which calls the function
